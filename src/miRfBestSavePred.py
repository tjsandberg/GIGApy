# Special version that does PredLon first and uses the output as input into PredLat
...
try:
    ...
    for tc in ["PredLon_24", "PredLat_24"]:
        print("\n" + "=" * 70)
        ...
        for n_features in feature_counts:
            ...
            if test_r2 > best_test_r2:
                ...
                if tc == "PredLon_24":
                    model.fit(X, y)
                    # Convert Series to DataFrame for later merge with X
                    best_PredLon_24_df = pd.DataFrame({
                        'best_PredLon_24': model.predict(X)  # Predict Lon using all features
                    })
                """
        ...
        latOrLon = "Latitude" if tc == "PredLat_24" else "Longitude"

        actVsPred = pd.DataFrame({
            #'test': X_test['Test'],
            #f'actual_{tc}': best_y_test[f'{tc}'],
            f'Abs_{latOrLon}': X_test[latOrLon],
            f'actual_{tc}': best_y_test,
            f'predicted_{tc}': best_test_pred
        })
